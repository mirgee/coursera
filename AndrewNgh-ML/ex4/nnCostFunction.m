...
z2 = Theta1*a1 % hidd_layer_size x m
a2 = [ones(1,m); sigmoid(z2)] % hidd_layer_size+1 x m
z3 = Theta2*a2 % num_labels x m
h = sigmoid(z3)
...
J = sum(sum(-yy.*log(h) - (one-yy).*log(one-h)))/m
...
%     yy(y(i)) = 1
%     for j = 1:num_labels
%         sum = sum - yy(j).*log(h(logical(yy(j)),i)) - ...
%             (1-yy(j)).*log(1-h(logical(yy(j)),i))
%     end
% end
% J = sum/m
